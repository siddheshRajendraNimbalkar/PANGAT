...
const Member = ({ data,channelId,convesation }: { data: MemberProps[],channelId:string,convesation:boolean }) => {
        ...
        <div>
            {
                data?.length == 0 ? null :
                    <div>
                        {
                            convesation ?
                            null : <h1 className="text-lg bg-indigo-900  text-zinc-200 pt-2 pb-2 flex justify-between items-center px-2">
                            <div>
                                Members
                            </div>
                        ...
                        }
                        {
                            data?.map((member) => (
                                <div className="flex justify-between items-center gap-x-2 hover:bg-zinc-700/10 dark:hover:bg-zinc-900/50 h-12 px-3 transition" key={member.id}
                                    onClick={() => {
                                        convesation ? route.push(`/channels/${channelId}/conversation/${member.id}`) : null
                                    }}
                                >
